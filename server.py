...
from packet import Command, Packet, Response
...
trackertimegauge = Gauge(
    "tracker_last_data_update",
    "tracker last data timestamp",
    ["device_id"] + list(promlabels.keys()),
)
locktimegauge = Gauge(
    "lock_last_data_update",
    "lock last data timestamp",
    ["device_id"] + list(promlabels.keys()),
)
...
class OmniLockProtocol(LineReceiver):
    ...
    def handleHeartbeat(self, data):
        ...
        update = {"device_id": self.device_id, "battery_voltage": data.data.voltage}
        print(jsons.dumps(update))
        resp = requests.post(ENDPOINT, headers=headers, data=jsons.dumps(update))
        print(resp)
        print(resp.text)
